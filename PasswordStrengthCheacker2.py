...
def checker(password):
    alphabets = "abcdefghijklmnopqrstuvwxyz"
    checklower = any(i in alphabets for i in password)   #checks if the password contain any lower case letter if so it return True
    checkupper = any(i in alphabets.upper() for i in password)   #checks if the password contain any upper case letter if so it return True
    checknumber = any(i.isdigit() for i in password)     #checks if it contain any digit if so, return True else False
    checksymbol = any(i in string.punctuation for i in password)   #using built-in function (string) to check if it contain any symbol if so return True
    sizeofPassword = len(password)
